...
export default function HaritaIcerik({ konuMeta, tumKonular, matrisData, iller }: HaritaIcerikProps) {
  const router = useRouter();
  const [secilenIlSlug, setSecilenIlSlug] = useState<string | null>(null);

  const secilenIlData = secilenIlSlug ? iller.find(i => i.slug === secilenIlSlug) ?? null : null;
        ...
        <div className="flex flex-wrap gap-2.5 pb-2">
          {tumKonular.map(k => {
            ...
            return (
              ...
              >
                <span className={`text-lg transition-transform group-hover:scale-125 ${isActive ? 'scale-110' : ''}`}>{k.icon}</span>
