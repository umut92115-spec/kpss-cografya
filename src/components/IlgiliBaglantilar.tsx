...
export default async function IlgiliBaglantilar({ tip, slug }: IlgiliBaglantilarProps) {
  ...
  if (tip === "il") {
    ...
    if (!il) return null;
    const bolgeUrl = `${il.bolge_slug}bolgesi`;
  ...
  if (tip === "konu") {
    ...
    return (
      <div className="mt-12">
        <h3 className="font-bold text-gray-800 text-xl mb-4 border-b pb-2">
          İllere Göre {slug.replace(/-/g, " ")}
        </h3>
