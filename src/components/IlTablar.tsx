...
export default function IlTablar({ params_slug, bolge_slug, tumKonular, konuVerileri }: IlTablarProps) {
          ...
          {aktifVeri ? (
                ...
                <div className="space-y-6">
                  ...
                  {'maden_turleri' in aktifVeri && (
                    <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                      <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">🛠️ Maden Türleri</h3>
                      <div className="flex flex-wrap gap-2">
                        {aktifVeri.maden_turleri.map(m => (
                          <span key={m} className="px-3 py-1 bg-amber-50 border border-amber-200 text-amber-800 rounded-lg text-xs font-bold shadow-sm">
                            {m}
                          </span>
                        ))}
                      </div>
                      {aktifVeri.onemli_not && (
                        <p className="mt-4 text-gray-600 text-sm leading-relaxed italic border-l-2 border-amber-200 pl-3">{aktifVeri.onemli_not}</p>
                      )}
                    </div>
                  )}
                  ...
                  {'ana_urunler' in aktifVeri && (
                    <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                      <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">🌾 Ana Ürünler</h3>
                      <div className="flex flex-wrap gap-2">
                        {aktifVeri.ana_urunler.map(u => (
                          <span key={u} className="px-3 py-1 bg-green-50 border border-green-200 text-green-800 rounded-lg text-xs font-bold shadow-sm">
                            {u}
                          </span>
                        ))}
                      </div>
                      {aktifVeri.ihracat_urunu && (
                        <p className="mt-4 text-sm text-gray-600 font-medium">
                          🚢 İhracat: <span className="text-green-700 underline decoration-green-200">{aktifVeri.ihracat_urunu}</span>
                        </p>
                      )}
                    </div>
                  )}
                  ...
                  {aktifVeri.kpss_notu && (
                    <div className="bg-orange-50 border border-orange-100 p-4 rounded-xl">
                      ...
                      <p className="text-orange-900 text-sm leading-relaxed font-medium">{aktifVeri.kpss_notu}</p>
                    </div>
                  )}
                </div>
                ...
                <div className="flex flex-col">
                  {aktifVeri.super_detay?.snippet ? (
                    ...
                    <div className="bg-gray-50/50 rounded-2xl p-6 border border-dashed border-gray-200 flex items-center justify-center text-center flex-1">
                      <p className="text-xs text-gray-400 italic">Bu konu hakkında detaylı özet içerik <br/>yakında eklenecektir.</p>
                    </div>
                  )}
                </div>
              </div>
                ...
                <div className="mt-8 pt-8 border-t border-gray-100">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="text-sm font-bold text-gray-800 flex items-center gap-2">
                      <span className="w-8 h-8 rounded-lg bg-blue-600 text-white flex items-center justify-center text-xs">?</span>
            ...
            <div className="py-12 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200">
               ...
               <p className="text-gray-400 italic text-sm">
                 Bu konuda {aktifKonu.kisa_baslik.toLowerCase()} için detaylı KPSS analizi henüz hazır değil.
               </p>
            </div>
          )}
