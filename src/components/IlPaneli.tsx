          ...
          <div className="space-y-8">
            ...
            {'maden_turleri' in konuData && (
              <div className="animate-fade-in delay-100">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="font-bold text-slate-800 uppercase tracking-wide text-xs">Yeraltı Zenginlikleri</h3>
                </div>
                <div className="flex flex-wrap gap-2">
                  {konuData.maden_turleri.map(m => (
                    <span key={m} className="px-4 py-1.5 bg-amber-50 text-amber-900 border border-amber-100 rounded-xl text-sm font-bold shadow-sm">
                      {m}
                    </span>
                  ))}
                </div>
            ...
            {'ana_urunler' in konuData && (
              <div className="animate-fade-in delay-200">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="font-bold text-slate-800 uppercase tracking-wide text-xs">Öne Çıkan Üretim</h3>
                </div>
                <div className="flex flex-wrap gap-2">
                  {konuData.ana_urunler.map(m => (
                    <span key={m} className="px-4 py-1.5 bg-emerald-50 text-emerald-900 border border-emerald-100 rounded-xl text-sm font-bold shadow-sm">
                      {m}
                    </span>
                  ))}
                </div>
            ...
            {'onemli_not' in konuData && konuData.onemli_not && (
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 animate-fade-in delay-300">
                <h3 className="font-bold text-slate-700 mb-1 text-xs uppercase tracking-widest">Coğrafi Not</h3>
            ...
            {konuData.kpss_notu && (
              <div className="bg-orange-500 text-white p-5 rounded-2xl shadow-lg shadow-orange-100 relative overflow-hidden group animate-fade-in delay-400">
                <div className="absolute -right-4 -top-4 text-6xl opacity-10 rotate-12 transition-transform group-hover:rotate-45 duration-500">🎓</div>
                <h4 className="font-black text-sm mb-2 flex items-center gap-2 uppercase tracking-tighter">
                  <span className="bg-white text-orange-600 w-5 h-5 rounded-full flex items-center justify-center text-[10px]">!</span>
                  KPSS Odaklı Bilgi
                </h4>
                <p className="text-sm font-medium leading-relaxed drop-shadow-sm">{konuData.kpss_notu}</p>
              </div>
            )}
