...
Soru ID'leri "${topic}-${batchId}-E1" formatında olsun. Kalıpları ÖSYM standartlarında, sadece kolay düzey doğrudan bilgi sorgusu yapacak şekilde kurgula.`;
      ...
      if (hardCount > 0) {
        const hardPrompt = `Lütfen '${topic}' konusuyla ilgili ${hardCount} adet soru üret. Dağılım: 5 adet "orta", geri kalanı "zor" seviye. Zor sorulardan en az 1-2 tanesi görselli veya Kalıp F (I, II, III) olsun.
İlgili konu verisi: ${geoDataChunk}
Soru ID'leri "${topic}-${batchId}-H1" formatında olsun.`;
